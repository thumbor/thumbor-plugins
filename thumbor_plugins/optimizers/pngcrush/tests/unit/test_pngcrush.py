...
from thumbor_plugins.optimizers.pngcrush import Optimizer


class PNGCrushOptimizerTest(TestCase):
    def setUp(self):
    ...
    def get_context(self):
        conf = Config()
        conf.PNGCRUSH_PATH = "/usr/bin/pngcrush"
    ...
    def test_should_run_mozjpeg_binary(self, pil_image_mock, subprocess_call_mock):
        ...
        subprocess_call_mock.assert_called_with(
            '/usr/bin/pngcrush -reduce -q input_file output_file', shell=True,
            stdin=mock.ANY,
        )
